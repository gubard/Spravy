...
public class EfAuthenticationService : IAuthenticationService
{
    ...
    )
    {
        ...
        switch (role)
        {
            case Role.User:
            {
                ...
                return context.GetEntityAsync<UserEntity>(id)
                   .IfSuccessAsync(userEntity => tokenFactory.Create(userEntity.ToUserTokenClaims()), ct);
            }
            case Role.Service:
            {
    ...
    public Cvtar UpdateVerificationCodeByLoginAsync(string login, CancellationToken ct)
    {
        ...
        return context.AtomicExecuteAsync(
            () => context.GetUserByLoginAsync(login, ct)
               .IfSuccessAsync(
                    userEntity =>
                    {
    ...
    public Cvtar UpdateVerificationCodeByEmailAsync(string email, CancellationToken ct)
    {
        ...
        return context.AtomicExecuteAsync(
            () => context.GetUserByEmailAsync(email, ct)
               .IfSuccessAsync(
                    userEntity =>
                    {
                        var verificationCode = randomString.GetRandom().ThrowIfNull();
    ...
    public ConfiguredValueTaskAwaitable<Result<bool>> IsVerifiedByLoginAsync(string login, CancellationToken ct)
    {
        login = login.Trim();

        return context.GetUserByLoginAsync(login, ct).IfSuccessAsync(user => user.IsEmailVerified.ToResult(), ct);
    }

    public ConfiguredValueTaskAwaitable<Result<bool>> IsVerifiedByEmailAsync(string email, CancellationToken ct)
    {
        email = email.Trim().ToUpperInvariant();
    ...
    public Cvtar VerifiedEmailByLoginAsync(string login, string verificationCode, CancellationToken ct)
    {
        login = login.Trim();
    ...
    public Cvtar VerifiedEmailByEmailAsync(string email, string verificationCode, CancellationToken ct)
    {
        email = email.Trim().ToUpperInvariant();
    ...
    public Cvtar UpdateEmailNotVerifiedUserByEmailAsync(string email, string newEmail, CancellationToken ct)
    {
        email = email.Trim().ToUpperInvariant();

        return context.AtomicExecuteAsync(
            () => context.GetNotVerifiedUserByEmailAsync(email, ct)
               .IfSuccessAsync(
                    userEntity =>
                    {
    ...
    public Cvtar UpdateEmailNotVerifiedUserByLoginAsync(string login, string newEmail, CancellationToken ct)
    {
        login = login.Trim();

        return context.AtomicExecuteAsync(
            () => context.GetNotVerifiedUserByLoginAsync(login, ct)
               .IfSuccessAsync(
                    userEntity =>
                    {
                        userEntity.Email = newEmail;
    ...
    private async ValueTask<Result> CreateUserCore(CreateUserOptions options, CancellationToken ct)
    {
        ...
        return await context.AtomicExecuteAsync(
            () => context.GetUserByLoginAsync(options.Login, ct)
               .IfSuccessAsync(_ => new(new UserWithLoginExistsError(options.Login)), ct)
               .IfErrorsAsync(
                    errors => errors.GetSingle("Errors")
                       .IfSuccess(
                            error =>
                            {
               ...
               .IfSuccessAsync(
                    () => context.GetUserByEmailAsync(options.Email, ct)
    ...
    {
        return hasherFactory.Create(entity.VerificationCodeMethod.ThrowIfNullOrWhiteSpace())
           .IfSuccess(
                newHasher =>
                {
                    var hash = newHasher.ComputeHash(code);
    ...
    {
        return hasherFactory.Create(entity.HashMethod.ThrowIfNullOrWhiteSpace())
           .IfSuccess(
                newHasher =>
                {
