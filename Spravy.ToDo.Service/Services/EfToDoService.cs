...
public class EfToDoService : IToDoService
{
    ...
    )
    {
        ...
        return dbContextFactory.Create()
           .IfSuccessDisposeAsync(context => context.AtomicExecuteAsync(() =>
                context.FindEntityAsync<ToDoItemEntity>(options.Id)
                   .IfSuccessAsync(item =>
                    {
                        ...
                        return CircleCompletionAsync(context, item, options.IsMoveCircleOrderIndex,
                                options.IsCompleteChildrenTask, options.IsOnlyCompletedTasks, cancellationToken)
                           .ConfigureAwait(false)
                           .IfSuccessAsync(
                                () => StepCompletionAsync(context, item, options.IsCompleteChildrenTask,
                                    cancellationToken),
                                cancellationToken);
                    }, cancellationToken), cancellationToken), cancellationToken);
    }
        ...
        return dbContextFactory.Create()
           .IfSuccessDisposeAsync(
                context => context.FindEntityAsync<ToDoItemEntity>(id)
                   .IfSuccessAsync(
                        item => getterToDoItemParametersService
                           .GetToDoItemParametersAsync(context, item,
                                httpContextAccessor.HttpContext.ThrowIfNull().GetTimeZoneOffset(), cancellationToken)
                           .IfSuccessAsync(
    ...
    )
    {
        ...
        return dbContextFactory.Create()
           .IfSuccessDisposeAsync(context => context.AtomicExecuteAsync(() =>
                context.FindEntityAsync<ToDoItemEntity>(options.ParentId)
