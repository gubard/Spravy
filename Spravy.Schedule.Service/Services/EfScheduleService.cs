...
public class EfScheduleService : IScheduleService
{
                            ...
                            items =>
                            {
                                ...
                                return items
                                   .IfSuccessForEachAsync(
                                        item => eventBusService.PublishEventAsync(item.EventId, item.Content, ct),
                                        ct
                                    )
                                   .IfSuccessAsync(() => context.RemoveRangeEntities(items), ct)
    ...
    public Cvtar RemoveTimerAsync(Guid id, CancellationToken ct)
    {
        return dbContextFactory.Create()
           .IfSuccessDisposeAsync(
                context => context.AtomicExecuteAsync(
                    () => context.GetEntityAsync<TimerEntity>(id).IfSuccessAsync(context.RemoveEntity, ct),
                    ct
                ),
                ct
            );
