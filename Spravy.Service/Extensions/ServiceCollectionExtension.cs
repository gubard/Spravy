...
{
    public static IServiceCollection AddSpravySqliteFolderContext<
        TDbContext,
        TAssemblyMark,
        TDbContextSetup
        ...
        where TDbContextSetup : IDbContextSetup
    {
        ...
        serviceCollection.AddTransient<
            ICacheValidator<string, TDbContext>,
            DbContextCacheValidator<TDbContext>
        >();

        serviceCollection.AddTransient(sp => new DbContextFactory<TDbContext, TAssemblyMark>(
            sp.GetRequiredService<TDbContextSetup>()
        ));

        serviceCollection.AddTransient<IFactory<string, TDbContext>>(sp => new CacheFactory<
            string,
            TDbContext
        >(
